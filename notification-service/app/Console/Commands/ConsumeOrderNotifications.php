...
class ConsumeOrderNotifications extends Command
{
    protected $signature = 'notifications:consume {--once : Process one available message and stop}';

    protected $description = 'Consume order events and send notification emails';

    public function handle(NotificationEventHandler $handler): int
    {
        ...
        $this->info('Waiting for order events. Press Ctrl+C to stop.');
    ...
    private function configureQueues(AMQPChannel $channel, array $rabbit): void
    {
        ...
        $channel->queue_bind($rabbit['queue'], $rabbit['exchange'], 'order.*');
