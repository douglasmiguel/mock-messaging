...
use App\Services\ConsumerHealthRecorder;
...
class ConsumeObservedEvents extends Command
{
    protected $signature = 'observability:consume {--once : Process one available event and stop}';

    protected $description = 'Consume all RabbitMQ domain events and record metrics';

    public function handle(RecordObservedEvent $recorder, ConsumerHealthRecorder $health): int
    {
        ...
        $consume = function (AMQPMessage $message) use ($recorder, $health, $channel, $rabbit): void {
            try {
                $event = json_decode($message->getBody(), true, 512, JSON_THROW_ON_ERROR);
                $recorded = $recorder->handle($event);
                $message->ack();
                $this->line($recorded ? 'Recorded '.($event['event_type'] ?? 'event') : 'Ignored duplicate event');
            } catch (\Throwable $exception) {
                report($exception);
                $this->error('Event failed: '.$exception->getMessage());
                $health->recordFailure('observability-service', $exception->getMessage());
        ...
        $this->info('Waiting for all domain events. Press Ctrl+C to stop.');
    ...
    private function configureQueues(AMQPChannel $channel, array $rabbit): void
    {
        ...
        $channel->queue_bind($rabbit['queue'], $rabbit['exchange'], '#');
    ...
    private function retryOrDeadLetter(AMQPChannel $channel, array $rabbit, AMQPMessage $message, \Throwable $exception): void
    {
        ...
        if (! $exception instanceof \InvalidArgumentException && $retryCount < $rabbit['max_retries']) {
            ...
            $message->ack();
            $this->recordFailureIncident($message, 'retry_scheduled', $nextRetryCount, $exception);

            return;
        }

        $this->recordFailureIncident($message, 'dead_lettered', $retryCount, $exception);
        $message->nack(false);
    }

    private function recordFailureIncident(AMQPMessage $message, string $outcome, int $retryCount, \Throwable $exception): void
    {
        try {
            try {
                $event = json_decode($message->getBody(), true, 512, JSON_THROW_ON_ERROR);
            ...
            ConsumerIncident::query()->create([
                ...
                'order_id' => data_get($event, 'data.order.id'),
